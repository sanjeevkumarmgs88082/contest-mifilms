jQuery(document).ready(function ($) {
    ...
    var themeName = 'ct_confetti.' + pathname;
    $('head').append('<link rel="stylesheet" type="text/css" href="../demo/css/demo.css">');
    $('head').append('<link rel="stylesheet" type="text/css" href="../demo/css/motive.css">');
    ...
    if ($('.ct-map').length)
      $('.ct-map').prepend('<p class="ct-switcher__error">Map colors can be changed only with <a href="http://codecanyon.net/item/jquery-shop-locator-/11541173?ref=createit-pl">Shop Locator plugin </a> settings (plugin is included with theme).</p>')


    /** style chooser show/hide **/
    jQuery("#stylechooser  #styleToggle").click(function () {
        setCookie('ct_confetti.switcher', $chooser.css('left') == '0px' || $chooser.css('left') == '-1px' ? 'hide' : 'show');
    ...
    function store() {
        ...
        setCookie('ct_confetti', r);
    }

    function load() {
        d = getCookie('ct_confetti');
        if (d) {
...
function selectDropdown(){
    ...
    if (getCookie('ct_confetti.dropdown') != null) {
        jQuery('.navbar-nav').removeClass('ct-navbar--fadeIn');
        jQuery('.navbar-nav').removeClass(getCookie('ct_confetti.dropdown'));
    } else{
        jQuery('.navbar-nav').removeClass('ct-navbar--fadeIn');
    }
    ...
    setCookie('ct_confetti.dropdown', strUser);
